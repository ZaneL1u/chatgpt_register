...
def test_cli_uses_tui_for_interactive_tty(monkeypatch, tmp_profiles_dir: Path) -> None:
    launched = {}
...
def test_cli_runs_batch_with_tui_result(monkeypatch, tmp_profiles_dir: Path, sample_mailtm_dict: dict) -> None:
    launched = {}
    config = RegisterConfig.model_validate(sample_mailtm_dict)

    monkeypatch.setattr(cli.sys, "stdin", _TTY())
    monkeypatch.setattr(cli.sys, "stdout", _TTY())
    monkeypatch.setattr(cli, "_launch_tui", lambda profile_manager: config)
